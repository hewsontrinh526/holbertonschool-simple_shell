 ...
 * @str: A string that may be associated with the command, and is freed
 * if a built-in command is found.
 * @status: A pointer to an integer representing the exit status.
 *
 * Return: 1 if a built-in command is executed, 0 if the command is not a
 * built-in command.
 */
...
/**
 * _opendir - Opens a directory with the given name and returns
 * a pointer to the directory stream.
 *
 * @name: A string representing the name of the directory to be opened.
 *
 * Return: Pointer to the directory stream on success. NULL if the
...
/**
 * _readdir - Reads a directory entry from the directory stream
 * pointed to by dp using the readdir function.
 *
 * @dp: A pointer to a directory stream.
 *
 * Return: Pointer to the directory entry structure if an entry is
 * read, else NULL if there are no more entries or an error occurs.
 */
